...
public class MySQLDatabaseDialect implements DatabaseDialect {
    ...
    public String areColumnsEqual(Column target, Column current) {
        if (!areTypesEqual(target.getType(), current.getType())) {
            return Strings.apply("Different column types: %s vs. %s",
                                 SchemaTool.getJdbcTypeName(target.getType()),
                                 SchemaTool.getJdbcTypeName(current.getType()));
        }
        if (target.isNullable() != current.isNullable()) {
            if (target.getType() != Types.TIMESTAMP || target.getDefaultValue() != null) {
                return "Not-null settings are different";
            }
        }
        if (!equalValue(target.getDefaultValue(), current.getDefaultValue())) {
            ...
            if (target.getType() != Types.TIMESTAMP || target.getDefaultValue() != null) {
                return Strings.apply("Different default values: %s vs. %s",
                                     target.getDefaultValue(),
                                     current.getDefaultValue());
            }
        }
        if (areTypesEqual(Types.CHAR, target.getType())) {
            if (!Objects.equal(target.getLength(), current.getLength())) {
                return Strings.apply("Different column length: %s vs. %s", target.getLength(), current.getLength());
            }
        }
        if (areTypesEqual(Types.DECIMAL, target.getType())) {
            if (!Objects.equal(target.getPrecision(), current.getPrecision())) {
                return Strings.apply("Different column precision: %s vs. %s",
                                     target.getPrecision(),
                                     current.getPrecision());
            }
            if (!Objects.equal(target.getScale(), current.getScale())) {
                return Strings.apply("Different column scale: %s vs. %s", target.getScale(), current.getScale());
            }
        }
        return null;
    }
    ...
    private boolean in(int type, int other, Integer... types) {
        return ArrayUtils.contains(types, type) && ArrayUtils.contains(types, other);
    }
    ...
    private String getTypeName(int type, int length, int precision, int scale) {
        ...
        throw new IllegalArgumentException(Strings.apply("Unknown column type: %s", SchemaTool.getJdbcTypeName(type)));
    }
    ...
    public int getJDBCType(Class<?> clazz) {
        ...
        throw new IllegalArgumentException(Strings.apply("Invalid column type: %s", clazz));
    }
